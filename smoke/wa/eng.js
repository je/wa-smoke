...
$("#districts-btn").click(function() {
  $.getJSON("/smoke/wa/districts/", function (data) {
    var title = data.features.length + " <strong>Districts</strong> <span class='small'><strong><quiet>(choose one)</quiet></strong></span>";
    var rows = "<tr><th>forest</th><th>district</th></tr>";
    var next = "</table></div><div class='col-xs-12 col-sm-6 col-md-6 col-lg-6'><table class='table table-striped table-bordered table-condensed small'>";
    for (var i in data.features) {
      row = "<tr><td>" + data.features[i].properties.F + "</td><td><strong><a href='/smoke/wa/?district=" + data.features[i].properties.C + "'>" + data.features[i].properties.D + "</a></strong></td></tr>";
      if (i >= data.features.length/2-1 && i < data.features.length/2) {
        row = row+= next 
      }
      rows += row
    }
    var content = "<div class='row'><div class='col-xs-12 col-sm-6 col-md-6 col-lg-6'><table class='table table-striped table-bordered table-condensed small'>" + rows + "</table></div></div>";
    var footer = "<span class='pull-left'><small></small></span><button type='button' class='btn btn-default' data-dismiss='modal'>Close</button>";
  $("#districts-title").html(title);
  $("#districts-info").html(content);
  $("#districts-footer").html(footer);
  })
...
var positions = L.geoJson(null, {
  ...
  onEachFeature: function (feature, layer) {
    if (feature.properties) {
      ...
      var title = "<quiet>Prescribed Burn</quiet> <span class='url-break " + c + "'>" + feature.properties.N  + "</span> #<span class='url-break " + c + "'>" + feature.properties.U + "</span><br><small><quiet>Unit</quiet> <strong>" + feature.properties.F + "</strong> <quiet>Subunit</quiet> <strong>" + feature.properties.D + "</strong>";
...
$.getJSON(yurl, function (data) {
  if (adist !== 'undefined' && adist !== null) {
    if (data.properties.f != data.properties.d) {
      dhead = "<small>@ " + data.properties.f + " &sect; " + data.properties.d + "</small>";
    } else {
      dhead = "<small>@ " + data.properties.d + "</small>";
    }
    $("#district-header").html(dhead);
  }
